...
export class AuthService {
  ...
  async validateUser(user: UserInput): Promise<User | false> {
    ...
    return (await bcrypt.compare(shaHashedPassword, foundUser.passwordHash))
      ? foundUser
      : false;
  }
  ...
  #fakeCheck = async (password: string): Promise<void> => {
    const fakePasswordHash =
      '$2b$10$uV.Pr.ov6HRcQbo1yJTi3.GEAsmkp.rSziGJs62IlmKOfM.20CPpi';
    await bcrypt.compare(password, fakePasswordHash);
  };

  #hashShaPassword = (password: string): string =>
